...
namespace codinglearning
{
    public partial class Form1 : Form
    {
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            mainToolTip.IsBalloon = true;      // 둥근 말풍선 모양 설정
            mainToolTip.AutoPopDelay = 10000;  // 말풍선 유지 시간 (10초)
            mainToolTip.InitialDelay = 200;    // 마우스 올리고 0.2초 뒤에 표시

            // 검색 버튼 설명
            string searchBtnHelp = "입력한 난이도 범위와 키워드 조건에 맞는 문제를 Codeforces에서 검색합니다.";
            mainToolTip.SetToolTip(btnSearch, searchBtnHelp);

            // 초기화 버튼 설명
            string resetSearchHelp = "입력한 검색 조건과 현재 불러온 문제 목록을 모두 깨끗하게 비웁니다.";
            mainToolTip.SetToolTip(btnResetSearch, resetSearchHelp);

            // 전체 버튼 설명
            string searchAllHelp = "아무런 조건 없이 Codeforces의 최근 문제 목록(상위 50개)을 그대로 가져옵니다.";
            mainToolTip.SetToolTip(btnSearchAll, searchAllHelp);

            // 문제 보기 버튼 설명 (우측 하단)
            string viewProbHelp = "목록에서 선택한 문제의 원본 웹페이지를 새 창에서 엽니다.\n문제를 읽고 복사할 수 있습니다.";
            mainToolTip.SetToolTip(btnViewProblem, viewProbHelp);

            // 난이도 라벨 설명
            string diffHelpText = "Codeforces 난이도(Rating) 안내\n" +
                                  "• 800 ~ 1199: Newbie (초보)\n" +
                                  "• 1200 ~ 1399: Pupil (입문)\n" +
                                  "• 1400 ~ 1599: Specialist (숙련)\n" +
                                  "• 1600 이상: Expert (전문가)\n\n" +
                                  "숫자가 높을수록 어려운 문제입니다.";
            mainToolTip.SetToolTip(lblDifficulty, diffHelpText);
            ...
            // 잔디 심기 (GitHub Push) 라벨 툴팁 (요청하신 대로 기존 코드 유지)
